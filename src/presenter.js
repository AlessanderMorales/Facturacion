...
// Importación de los creadores concretos
...
// Función para convertir números a letras (la misma que tenías)
...
// ===== Cargar productos del backend y agregar al carrito =====
...
window.agregarCarrito = (id, nombre, precio) => {
    ...
    const itemVenta = new ItemVenta(cantidad, producto);
    ventaActual.agregar_item(itemVenta);
...
metodoPagoSelect.addEventListener("change", (e) => {
    ...
    if (metodo === "qr") {
        ...
        const qrData = `Total: ${ventaActual.calcular_total()} Bs.`;
        const canvas = document.createElement("canvas");
        QRCode.toCanvas(canvas, qrData).then(c => qrContainer.appendChild(c));
...
confirmarPagoBtn.addEventListener("click", async () => {
    ...
    const nitCliente = document.getElementById("cliente-ci").value.trim();
    if (!nombreCliente || !nitCliente) return alert("Ingrese nombre y CI/NIT");
    if (ventaActual.items.length === 0) return alert("Agrega productos al carrito");

    const metodo = metodoPagoSelect.value;
    let creador;
    let pago;
    const montoTotal = ventaActual.calcular_total();
    const montoLetras = numeroALetras(montoTotal);

    // Aplicación del Factory Method
    switch (metodo) {
        case "cash":
            creador = new CreadorPagoCash();
            pago = creador.crearPago(montoTotal, montoLetras);
            break;
        case "tarjeta":
            const numeroTarjeta = document.getElementById("numero-tarjeta").value;
            if (numeroTarjeta.length !== 16) return alert("Número de tarjeta inválido (16 dígitos)");
            creador = new CreadorPagoTarjeta();
            pago = creador.crearPago(montoTotal, montoLetras, numeroTarjeta);
            break;
        case "qr":
            const qrCodeData = `Factura N°${Math.floor(Math.random()*10000)} | Cliente: ${nombreCliente} | Total: ${montoTotal} Bs.`;
            creador = new CreadorPagoQR();
            pago = creador.crearPago(montoTotal, montoLetras, qrCodeData);
            break;
        default:
    ...
    const tienda = new Tienda(tiendaData.nombre_tienda, tiendaData.ubicacion, tiendaData.telefono, tiendaData.codigo_autorizacion, tiendaData.nit);
    const cliente = new Cliente(nitCliente, nombreCliente, nitCliente);

    const factura = new Factura(Math.floor(Math.random() * 10000), new Date(), ventaActual, pago, tienda, cliente);

    facturaDiv.innerHTML = `
        <h2>Factura N° ${factura.numero_factura}</h2>
        <p>Tienda: ${factura.tienda.nombre_tienda}</p>
        <p>Ubicación: ${factura.tienda.ubicacion}</p>
        <p>Teléfono: ${factura.tienda.telefono}</p>
        <p>Cliente: ${factura.cliente.nombreYApellido} (CI/NIT: ${factura.cliente.nit})</p>
        <p>Detalle:</p>
        <ul>${factura.venta.items.map(i => `<li>${i.cantidad} x ${i.producto.nombre} = ${i.subtotal} Bs.</li>`).join("")}</ul>
        <p>Total: ${factura.total} Bs. (${factura.pago.monto_en_letras})</p>
        <p>Método de pago: ${pago.constructor.name}</p>
    ...
    fetch("http://localhost:3000/facturas", {
        ...
        body: JSON.stringify({
            ...
            fecha: factura.fecha,
            cliente: factura.cliente.nombreYApellido,
            nit: factura.cliente.nit,
            detalle: factura.venta.items.map(i => `${i.cantidad} x ${i.producto.nombre} = ${i.subtotal} Bs.`).join(" | "),
            total: factura.total,
            metodo_pago: factura.pago.constructor.name
        })
...
document.getElementById("ver-historial-btn").addEventListener("click", async () => {
    ...
    facturas.forEach(f => {
        ...
        li.innerHTML = `<b>Factura #${f.numero_factura}</b> - ${f.fecha}<br>
            Cliente: ${f.cliente} (CI/NIT: ${f.nit})<br>
            Detalle: ${f.detalle}<br>
            Total: ${f.total} Bs. (${numeroALetras(f.total)}) | Pago: ${f.metodo_pago}<br>
